...
def step_decay(step):
  curr = tf.cast(step, tf.float32)
  warmup_steps =4000
  d_model = 512
  d_model = tf.cast(d_model, tf.float32)
  arg1=  tf.math.rsqrt(curr)
  arg2 = tf.scalar_mul((warmup_steps ** -1.5),curr)
...
class CorefModel(object):
  def __init__(self, config):
    ...
    self.seq_length = 500
    self.new_dim = 512
    self.sample_transformer = Transformer( num_layers=2, d_model=self.new_dim, num_heads=8, dff=2048, input_vocab_size=self.seq_length, target_vocab_size=self.seq_length)
    ...
    learning_rate1 = tf.train.exponential_decay(self.config["learning_rate"], self.global_step,
    ...
    train_op1 = optimizer1.apply_gradients(zip(gradients1, var_list1), global_step=self.global_step)
    train_op2 = optimizer2.apply_gradients(zip(gradients2, var_list2), global_step=self.global_step)
  ...
  def start_enqueue_thread(self, session):
    ...
    import pickle
    with open('../dataset/train.pickle', 'rb') as handle:
      train_examples = pickle.load(handle)
    # with open('../dataset/train.pickle', 'wb') as handle:
  ...
  def get_predictions_and_loss(self, tokens, context_word_emb, head_word_emb, lm_emb, char_index, text_len, speaker_ids, genre, is_training, gold_starts, gold_ends, cluster_ids):
    ...
    with tf.name_scope("transformer"):
      ...
      padding_mask_partial = tf.cast(tf.sequence_mask(tf.shape(temp_input)[0], maxlen=self.seq_length),  dtype = tf.float32 ) 
      multiples  =  [self.seq_length]
      padding_mask_partial2  = tf.tile(padding_mask_partial, multiples)
      enc_padding_mask = tf.reshape(padding_mask_partial2, [ multiples[0],  -1 ])
      # enc_padding_mask  = tf.matrix_set_diag(enc_padding_mask, tf.zeros(enc_padding_mask.shape[0:-1]), name=None)
      
      dec_padding_mask = tf.reshape(padding_mask_partial2, [ multiples[0],  -1 ])
      dec_padding_mask  = tf.matrix_set_diag(dec_padding_mask, tf.zeros(dec_padding_mask.shape[0:-1]), name=None)

      look_ahead_mask = create_look_ahead_mask(tf.shape(padding_mask_partial)[0])
      ...
      embd = predictions[:k]
      # embd =  tf.placeholder(dtype=tf.float16,shape=[None,50])
      # k = tf.shape(embd)[0]
      ...
      k_reshape = tf.reshape(k , [1,])
      all_indices = tf.tile(top_span_range , k_reshape)
      top_antecedents = tf.reshape(all_indices, [k,-1])
      top_antecedent_emb_ = tf.gather(embd, top_antecedents) # [k, k, emb]
  ...
  def create_padding_mask(self, seq , seq_length  ): 
  ...
  def load_eval_data(self):
      import pickle
      def load_line(example):
        return self.tensorize_example(example, is_training=False), example
      
      with open('../dataset/test.pickle', 'rb') as handle:
        test = pickle.load(handle)
    
      self.eval_data = [load_line(example) for example in test]
