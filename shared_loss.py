...
class CorefModel(object):
    ...
    def tensorize_example(self, example, index, is_training):
        ...
        if is_training:
            ...
            swag_embedding = pickle.load(open(self.swag_val_dir + swag_file, "rb"))
    ...
    def get_predictions_and_loss_cm(self, tokens, context_word_emb, head_word_emb, lm_emb, text_len,\
        ...
        swag_context_emb = tf.concat([swag_context_emb, swag_context_emb],2)
        print("====", swag_context_emb)
        context_outputs = self.lstm_contextualize_multitask(swag_context_emb, swag_text_len, swag_len_mask)# [5, max_sq_len, emb]
        with tf.variable_scope("yolo_common_sense"):
